...
--- Significa que el servidor no tiene memoria. Cada vez que haces clic o abres una página, el servidor no se acuerda de quién eres ni de lo que hiciste un segundo antes.

### 2.

¿Por qué `request.form` no está disponible después de un `redirect()`?

--- Porque un redirect() fuerza al navegador a hacer una visita completamente nueva a otra página. En esa nueva visita, los datos que se escribieron en el formulario ya no viajan
...
--- request.form: Captura lo que el usuario acaba de escribir en el formulario (solo dura un instante).

session: Es la memoria donde guardas ese dato para poder usarlo en varias páginas.
...
--- Es una contraseña que sirve como sello de seguridad. Evita que los usuarios puedan alterar o falsificar la información guardada en la sesión desde su propio navegador.

### 5.

¿Por qué no deberíamos utilizar `session` como sustituto de una base de datos?

--- Porque la sesión es temporal y muy reducida. Si el usuario borra su historial/cookies o cierra el navegador, todos los datos guardados en la sesión se pierden para siempre.

### 6.

¿Qué tipo de información tendría sentido almacenar temporalmente en una sesión?

--- El nombre o ID del usuario que ya inició sesión.

El idioma seleccionado o modo oscuro.

Mensajes breves de aviso (como "Guardado con éxito").
